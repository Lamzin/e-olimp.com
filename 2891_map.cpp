...
int main(){
	...
	for(int i = 0, v = 0; i < s.length() && v != -1; i++){
		v = (trie[v].next.find(new_ch[s[i]]) != trie[v].next.end()) ? trie[v].next[new_ch[s[i]]] : -1;
	...
	for (int i = 0; i < s.length(); i++){
		if (prefix[i]){
			for(int j = i + 1, v = 0; v != -1 && j < s.length(); j++){
				v = (trie[v].next.find(new_ch[s[j]]) != trie[v].next.end()) ? trie[v].next[new_ch[s[j]]] : -1;
